...
<?php
	...
	$v["raw_post_json_parsed"] =  json_decode($v["raw_post_unparsed"],true); 
	...
	try {
        if (file_put_contents($rawfile, json_encode($v))) {
            echo ("\nOK - rawfile.dump\n");
		}
		else {
			echo ("\nrawfile.dump not written\n");
		};
    ...
    try { // packet only but save as device sernum
        ...
        $fname = "./data/" . $vx["samp_data"]["meta"]["device_sernum"] . "_kdata.json";
